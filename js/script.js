...
for(let i=0;i<dropList.length;i++){
    ...
    {
        //selecting USD(from) and ZAR(to) as default
        let selected;
        if(i==0)
        {
      ...
      let option='<option value='+currency_code+'>'+currency_code+'</option>';
...
function getExchangeRate(){
    ...
    //if value is not entered or it is 0,it is reset to 1 by default
    if(amountValue=="" || amountValue=="0")
    {
        amount.value="1";
        amountValue=1;
    }
    let url='https://v6.exchangerate-api.com/v6/6be43c88c6b18f29270fff56/latest/${fromCurrency.value}';
...
for (let i = 0; i < dropList.length; i++) {
    for(let currency_code in country_code){
        let selected = i == 0 ? currency_code == "USD" ? "selected" : "" : currency_code == "ZAR" ? "selected" : "";
